...
def main(args):
    ...
    image_path = "/media/parvez/Expansion/exp_data/training/image_2/000000.png"
    #print("pcd = ", pc.shape)   
    images = [] 
    ...
    # add the LISA Noise 
    lisa = LISA(atm_model="strong_advection_fog")  

    # augmentation 
    batched_pts = pc
    batched_pts = lisa.augment(batched_pts) # rain rate 33 mm/hr 
    ...
    with torch.no_grad():
        if not args.no_cuda:
            pc_torch = pc_torch[:, 0:4].float().cuda() 

            #print("*******pc_torch shape =", pc_torch.shape)
        
        result_filter = model(batched_pts=[pc_torch], 
                              mode='test', images=images)[0]
