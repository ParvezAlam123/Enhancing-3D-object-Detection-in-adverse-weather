...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    ids = list(sorted(det_results.keys()))
    for id in ids:
...
def main(args):
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            images = [] 
            for q in range(len(data_dict["batched_img_info"])):
                img_info_dict = data_dict["batched_img_info"][q]
                image_path = os.path.join(args.data_root, img_info_dict["image_path"]) 
            ...
            # augmentation 
            batched_pts = batched_pts[0].cpu().numpy() 
            batched_pts = lisa.augment(batched_pts, 100) # rain rate 33 mm/hr 
            batched_pts = [torch.from_numpy(batched_pts[:, 0:4]).float().cuda()]
   

            batched_gt_bboxes = data_dict['batched_gt_bboxes']
            batched_labels = data_dict['batched_labels']
            batched_difficulty = data_dict['batched_difficulty'] 
            #print("****************batched pts shape = ", batched_pts[0].shape)
