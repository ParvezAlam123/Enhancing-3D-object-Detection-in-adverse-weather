...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        for weights in self.dinov2_model.parameters():
            weights.requires_grad = False  

        


        self.backbone = Backbone(in_channel=832, 
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None, images=None):
        batch_size = len(batched_pts)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        img_tensor = [] 
        # get the dinov2 features for every image in 
        for i in  range(len(images)):
            if i == 0:
               ...
               img_tensor = last_hidden_states 
            else:
                img = images[i]
                inputs = self.processor(images=img, return_tensors="pt").to(device)
                outputs = self.dinov2_model(**inputs)
                last_hidden_states = outputs[0] 
                img_tensor = torch.cat((img_tensor, last_hidden_states), dim=0) 



        # take the global features using maxpool [1, 257, 768] -> [1, 1, 768]
        #print("images = ", img_tensor.shape)

        global_features_img = torch.max(img_tensor, dim=1, keepdim=True)[0].to(device)
        global_features_img = global_features_img.unsqueeze(dim=1) 
        #print("global_features = ", global_features_img.shape) 
        ...
        # fused the pillar features with img features 
        B, d, H, W = pillar_features.shape
        global_features_img = global_features_img.repeat(1, H, W, 1)
        pillar_features = pillar_features.permute(0, 2, 3, 1) 

        #print("pillar_features = ", pillar_features.shape, "global_featuers_img", global_features_img.shape)

        pillar_features = torch.cat((pillar_features, global_features_img), dim=-1).permute(0, 3, 1, 2) 


        print("pillar_features = ", pillar_features.shape)
